...
async function runTest() {
  ...
  if (!ANTHROPIC_KEY && !GEMINI_KEY) {
    console.error(
      'ERROR: No API key found. Set ANTHROPIC_API_KEY or GEMINI_API_KEY environment variable.'
    );
    return;
  }
  ...
  try {
    const result = await extractRelationsFromStorySession(tennisStory, existingPeople, config);
    ...
    const davidAmbiguous = result.ambiguousMatches?.find((am) =>
      am.nameInStory.toLowerCase().includes('david')
    );
    console.log(
      `  ${davidAmbiguous ? '✅' : '❌'} David flagged as ambiguous: ${!!davidAmbiguous}`
    );

    const olaAmbiguous = result.ambiguousMatches?.find((am) =>
      am.nameInStory.toLowerCase().includes('ola')
    );
    console.log(`  ${olaAmbiguous ? '✅' : '❌'} Ola flagged as ambiguous: ${!!olaAmbiguous}`);

    const davidInPeople = result.people.find(
      (p) => p.name.toLowerCase().includes('david') && !p.isNew
    );
    console.log(
      `  ${!davidInPeople ? '✅' : '❌'} David NOT auto-linked to existing: ${!davidInPeople}`
    );
